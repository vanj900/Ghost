...
# Stores emotions, events, and context in a RAM-backed SQLite database.

set -euo pipefail

# Use a RAM-backed filesystem when available, fall back to /tmp
if [[ -d /dev/shm ]]; then
...
ghost_memory_init() {
  if ! command -v sqlite3 &>/dev/null; then
    echo "[ghostmemory] WARNING: sqlite3 not found — memory disabled." >&2
    return 1
  fi
...
SQL
  echo "[ghostmemory] Diary initialised at ${GHOST_MEMORY_DB}"
}
...
ghost_memory_add() {
  ...
  sqlite3 "$GHOST_MEMORY_DB" \
    "INSERT INTO diary(kind,content,emotion,mask) VALUES('$ek','$ec','$ee','$em');" 2>/dev/null || return 0
}
...
ghost_memory_recent() {
  ...
  sqlite3 "$GHOST_MEMORY_DB" \
    "SELECT datetime(ts,'unixepoch','localtime')||' ['||kind||'/'||mask||'] '||content
     FROM diary ORDER BY id DESC LIMIT ${n};" 2>/dev/null || true
}

ghost_memory_count() {
  command -v sqlite3 &>/dev/null || { echo "0"; return 0; }
  sqlite3 "$GHOST_MEMORY_DB" "SELECT COUNT(*) FROM diary;" 2>/dev/null || echo "0"
}
...
# Entry point when run directly — quick self-test
